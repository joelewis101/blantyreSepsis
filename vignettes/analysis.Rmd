---
title: "Aetiology and determinants of outcome in sepsis in an urban african centre: an observational cohort study"
...
Joseph M Lewis^1,2,3^, , Madalitso Mphasa^1^, Lucy Keyala^1^, Rachel Banda^1, Emma Smith^1^,^2^,  Jackie Duggan^4^, Tim Brooks^4^, Matthew Catton^4^, Jane Mallewa^5^, Grace Katha^5^, Brian Faragher^2^, Melita Gordon^1,3^, Jamie Rylance^1,2^, Nicholas A Feasey^1,2^
...
t2 %>%
  mutate(levels = case_when(
    ...
    select(-variable) %>% 
  kbl(caption = "SUPPLEMENTARY TABLE 2: Antimicroial therapies administered.",
...
BTaetiology %>% 
  ...
  dplyr::group_by(name) %>%
   dplyr::summarise(n_pos = sum(value == 1, na.rm = TRUE),
            ...
            positive_tests_prop_ci = paste0("(",sp_dc(n_positive_tests_lci*100,1),
                                           "-",
                                           sp_dc(n_positive_tests_uci*100,1),
                                           "%)"),
            n_prev= n_pos/225,
  ...
  kbl(caption = 
        "TABLE 2: Positive diagnostic tests and minimum disease prevalence.",
...
BTaetiology %>%
  select(hivstatus, tb, arbovirus, CrAg_LFA,
...
kbl( col.names = c("Diagnosis", "HIV+", "HIV-", "Difference"),
 caption = "TABLE 3: Diagnosis stratified by HIV status") %>%
kable_classic(full_width = FALSE) 
...
BTsera %>% 
  ...
  kbl(
    caption = "SUPPLEMENTARY TABLE 3: Acute and convalescent serology results", 
...
kbl(select(st4,-boldcol), 
      row.names = F, caption = "SUPPLEMENTARY TABLE 4: Bivariate associations with TB diagnosis. Numeric variables are presented as median (IQR) and categorical variables as proportions. Difference column shows difference in medians or difference in proportions with 95% confidence intervals. Variables shown in bold are those for which the 95% confidence intervals do not cross 0." ) %>%
 kable_classic(full_width = FALSE) %>%
...
BTaetiology %>% 
  ...
  kbl(caption = 
      "SUPPLEMENTARY TABLE 5: Positive test reaults stratified by HIV status",
      col.names = c("Test", 
                    "Non reactive",
                    "Reactive",
                    "Unknown"),
...
#jpeg(file = "figures/upset_fig_files/MAIN_upset_diagnoosis.jpeg", width = 8, height = 5.5, 
#     units = "in", res = 600)
#upsetplot
#dev.off()
#pdf(file = "figures/upset_fig_files/MAIN_upset_diagnoosis.pdf", width = 8, height = 5.5)
#upsetplot
...
#  geom_bar() + 
#  scale_fill_manual(values = c("gray23",viridis_pal( option = "C")(6)[4]),
#                    labels = c("No antibacterials", 
#                               "Received antibacterials")) +
#  theme(legend.title = element_blank()) -> skank

#ggsave("figures/upset_fig_files/hacky_legend_for_upset.pdf", skank)


```

```{r sf3, fig.width = 8, fig.height = 3,fig.cap = "Pathogens identified by (A) aerobic blood culture and (B) multiplex PCR on blood. 26 blood cultures in 24/224 participants grew 28 pathogens in total. 26 pathogen PCR targets were identified in 31/122 samples. n here is number of samples."}
...
p
#ggsave("figures/SUP_BC_PCR_dx.jpg", width = 8, height = 3, units = "in")
```




```{r sf4, fig.cap = "SUPPLEMENTARY FIGURE 4: number of participants in whom a bacterial pathogen was identified: aggregated (A) and to species level (B). 43 bacterial pathogens were identified in 38 participants. n here is number of participants.", fig.height = 4, fig.width = 10}
...
#ggsave("figures/SUP_bact_pathogens.jpeg", pb, height = 4, width = 10, units = "in")

```


```{r sf5, fig.cap = "SUPPLEMENTARY FIGURE 5: Euler diagrm shwoing overlapping diagnoses", fig.height= 4, fig.width=6}

# Euler diagram of overlapping diagnoses
      
d$`No diagnosis` = as.numeric(apply(d[-c(1,2)], 1, sum) == 0)
...
p1
#ggsave("figures/MAIN_euler_dx.jpg", width = 6, height =4, units = "in")
...
psurv2
#ggsave("figures/MAIN_KM_plot.jpeg", psurv2, width = 6, height = 4)

```

```{r sf6, fig.cap = "SUPPLEMENTARY FIGURE 6: Mortality stratified by diagnosis", fig.width = 5, fig.height = 3}
...
#ggsave("figures/SUP_mort_by_dx.jpeg", paetiol, width = 5, height = 3,
#       units = "in")
...
```{r sf9, fig.cap = "SUPPLEMENTARY FIGURE 9:Missing data by variable. HIV related variables were not available for HIV-noninfected participants, recorded here as \\'Not done (HIV-)\\'. SpO2 = capilliary oxygen saturation, SBP/DBP = systolic/diastolic blood pressure, RR = respiratory rate, TB = tuberculosis, WCC = white cell count, ART = antiretroviral therapy, CPT = cotrimoxazole preventative therapy.", fig.width  = 6, fig.height = 4}
...
#ggsave( "figures/miss_data.jpg",p.miss, width = 6, height = 4, units = "in")
...
```{r pca2, fig.cap = "SUPPLEMENTARY FIGURE 8: Original host-severity variable contributions to PC1 (A), PC2 (B)  and PC3 (C) and original variables projected onto PC3 and PC4 (D).", fig.width = 8, fig.height = 8  }
...
### Fit models for antimcrobial therapy
...
kbl(mod.output.tab,
    row.names = F, caption = "SUPPLEMENTARY TABLE 7: Parameter estimates from models assessing effect of therapies on mortality, expressed as adjusted odds ratios with a point estimate (posterior median) and 95% credible intervals.") %>%
  kable_classic(full_width = FALSE)
...
#ggsave( "figures/MAIN_mort_model.plot.jpeg",
#        mort.model.plot.final, 
